...
class Actor(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(12 * args.W * args.H + args.feature_len, args.hidden_width)
...
class PPO_discrete:
    ...
    def test_action(self, s):  # When evaluating the policy, we select the action with the highest probability
        X = np.reshape(s[0], (-1, 4, self.W, self.H))
        X = torch.FloatTensor(X)
        Y = torch.unsqueeze(torch.FloatTensor(s[1]), 0)
        a_prob = self.actor(X,Y).detach().numpy().flatten()
        a = np.argmax(a_prob)
        return a
    ...
    def choose_action(self, s):
        ...
        Y = torch.unsqueeze(torch.FloatTensor(s[1]), 0)
        with torch.no_grad():
            dist = Categorical(probs=self.actor(X,Y))
            a = dist.sample()
            a_logprob = dist.log_prob(a)
        return a.numpy()[0], a_logprob.numpy()[0]

    def update(self, replay_buffer, total_steps):
        xs,ys, a, a_logprob, r, xs_,ys_, dw, done = replay_buffer.numpy_to_tensor()  # Get training data
        """
        ...
        with torch.no_grad():  # adv and v_target have no gradient
            vs = self.critic(xs,ys)
            vs_ = self.critic(xs_,ys_)
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                dist_now = Categorical(probs=self.actor(xs[index],ys[index]))
                ...
                v_s = self.critic(xs[index],ys[index])
    ...
    def save_model(self,file,step):
        ...
        if not os.path.exists(f):
            os.makedirs(f)
        torch.save(self.actor.state_dict(), f +'/' + 'eval_net.pkl')
        torch.save(self.critic.state_dict(), f + '/' + 'target_net.pkl')

    def load_model(self,model_path):
        if os.path.exists(model_path + '/eval_net.pkl'):
            self.actor.load_state_dict(torch.load(model_path + '/eval_net.pkl'))
            self.critic.load_state_dict(torch.load(model_path + '/target_net.pkl'))
