...
def Dominate(Pop1,Pop2):
    ...
    if len(Pop1.fitness)==2:
        if (Pop1.fitness[0]<Pop2.fitness[0] and Pop1.fitness[1]<Pop2.fitness[1]) or \
            ...
            return False
    elif len(Pop1.fitness)==3:
        if (Pop1.fitness[0] < Pop2.fitness[0] and Pop1.fitness[1] < Pop2.fitness[1] and Pop1.fitness[2] < Pop2.fitness[
            2]) or \
                (Pop1.fitness[0] <= Pop2.fitness[0] and Pop1.fitness[1] < Pop2.fitness[1] and Pop1.fitness[2] <
                 Pop2.fitness[2]) or \
                (Pop1.fitness[0] < Pop2.fitness[0] and Pop1.fitness[1] <= Pop2.fitness[1] and Pop1.fitness[2] <
                 Pop2.fitness[2]) or \
                (Pop1.fitness[0] < Pop2.fitness[0] and Pop1.fitness[1] < Pop2.fitness[1] and Pop1.fitness[2] <=
...
def Plot_NonDominatedSet(EP,shape,G_label,xlable,ylable,title=None):
    if len(EP[0].fitness)==2:
        ...
        plt.xlabel(xlable,fontproperties='SimSun',fontsize=13)
        plt.ylabel(ylable,fontproperties='SimSun',fontsize=13)
    else:
        x = []
        y = []
        z = []
        ...
        ax.scatter(x, y, z)
        # 添加坐标轴(顺序是Z, Y, X)
        ax.set_zlabel('Max Machine Load')
        ax.set_ylabel('Total Machine Load')
...
def crowding_distance(NDSet):
    ...
    for i in range(1,len(ND)-1):
        if Distance[ND[i][0]]==0:
            # print(ND[i+1][1][0],ND[i-1][1][0],ND[i-1][1][2])
            for j in range(len(ND[i+1][1])):
                Distance[ND[i][0]]+=abs(ND[i+1][1][j]-ND[i-1][1][j])
